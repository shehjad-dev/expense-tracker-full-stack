...
export class ExpensesService {
        ...
        try {
            ...
            for (const expense of recurringExpenses) {
                try {
                    ...
                    this.logger.debug(`Created new recurring expense for ${expense.name}`);
                    ...
                    this.logger.error(`Error processing recurring expense ${expense.name}`, error);
                }
            }
    ...
    private getPaginationMeta(totalExpenses: number, page: number, limit: number, sortBy: string, expenseType?: string) {
        const sort = sortBy === 'oldest' ? '&sortBy=oldest' : '';
        const expenseTypeInQuery =
            expenseType === 'recurring' || expenseType === 'non-recurring' ? `&expenseType=${expenseType}` : '';
        ...
        return {
            ...
            totalPages,
            nextPage:
                totalPages > page ? `/expenses?${sort}${expenseTypeInQuery}&page=${page + 1}&limit=${limit}` : null,
    ...
    async findAll(
        ...
        try {
            const [paginatedExpenses, totalExpenses] = await Promise.all([
                this.expenseModel.find(query).select('-__v').sort(sort).skip(skip).limit(limit).exec(),
    ...
    async create(expense: CreateExpenseDto): Promise<{ message: string; newExpenseId: string }> {
        ...
        try {
            ...
            const category = await this.categoryModel.findOne({ name: expense.categoryName }).session(session);

            if (!category) {
                const newCategory = await this.categoryModel.create([{ name: expense.categoryName }], { session });
            ...
            throw new HttpException(error.message || 'Database operation failed', error.status || 500);
        } finally {
            session.endSession();
        }
    }

    async update(id: string, expense: UpdateExpenseDto): Promise<{ message: string; updatedExpense: Expense }> {
        if (!id.match(/^[0-9a-fA-F]{24}$/)) {
            throw new BadRequestException(`Invalid ID format: ${id}`);
        }
        ...
        try {
            ...
            if (!category) {
                ...
                updatedExpense = await this.expenseModel
                    .findByIdAndUpdate(id, expense, { new: true, session })
                    .exec();
            } else {
                updatedExpense = await this.expenseModel
                    .findByIdAndUpdate(id, expense, { new: true, session })
    ...
    async remove(id: string): Promise<{ message: string; deletedExpense: Expense }> {
        if (!id.match(/^[0-9a-fA-F]{24}$/)) {
            throw new BadRequestException(`Invalid ID format: ${id}`);
        }
